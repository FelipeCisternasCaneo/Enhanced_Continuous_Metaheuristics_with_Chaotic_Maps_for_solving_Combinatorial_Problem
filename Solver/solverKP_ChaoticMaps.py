...
def solverKP_ChaoticMaps(id, mh, maxIter, pop, instancia, DS, param):
    ...
    if chaotic == 'LOG':
        cantidad_elementos = maxIter * pop * instance.getItems()
    ...
    for iter in range(0, maxIter):
        ...
        # print(poblacion)
        if mh == "SCA":
            poblacion = iterarSCA(maxIter, iter, instance.getItems(), poblacion.tolist(), Best.tolist())
        if mh == "GWO":
        ...
        if mh == 'PSA':
            poblacion = iterarPSA(maxIter, iter, instance.getItems(), poblacion.tolist(), Best.tolist())
        if mh == "MFO":
            poblacion, bestSolutions = iterarMFO(maxIter, iter, instance.getItems(), len(poblacion), poblacion, bestSolutions, fitness, BestFitnessArray )
        if mh == "GA":
